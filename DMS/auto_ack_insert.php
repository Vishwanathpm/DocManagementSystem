...
/*if(!isset($_POST['version'])){
    $getversion = mysql_query("select versionid from files where fileid = $file");
    while ($x = mysql_fetch_array($getversion)){
        $version = $row[versionid];
    }
}*/

echo 'File:  '.$file.'  Version: '.$version.' Users: '.$requsers.'<br><br>';
...
//$sql3="SELECT * FROM users, files, acks WHERE afileicks.fileid = '$file' and acks.versionid = '$version' and users.userid=acks.userid and acks.fileid = files.fileid and acks.ackbit IS NULL and acksent = 0";
